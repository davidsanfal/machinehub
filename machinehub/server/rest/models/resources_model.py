import fnmatch
import os
from old_machinehub.common.errors import MachinehubException, NotMachineHub, ForbiddenException,\
    InvalidNameException
from old_machinehub.config import MACHINES_FOLDER, MACHINEFILE
import zipfile
import shutil
from old_machinehub.server.app.services.permission_service import user_can_edit
from old_machinehub.common.model.machine_name import MachineName
...
def save(resource, dest, current_user, extensions=None, pattern_extensions=None):
    if not resource:
        return None
    ...
    if allowed:
        ...
        try:
            machine_name = "{user}/{machine}".format(user=current_user,
                                                     machine=name)
...
def extract_zip(file_path, current_user):
    machine_name = None
    try:
                ...
                if not len(files_in_zip) == 1:
                    ...
                    if ext == '' and _name == '%s/' % name and \
                       all(s.startswith('%s/' % name) for s in files_in_zip):
                        z.extractall(os.path.join(MACHINES_FOLDER, current_user))
