...
from machinehub.util.files import _generic_algorithm_sum
...
def fetch_sourceforge_url(url):
    ...
    try:
        ...
        if not new_url:
            raise MachinehubException("File not found: %s" % url)
    return new_url
...
def check_with_algorithm_sum(algorithm_name, file_path, signature):
    ...
    if real_signature != signature:
        raise MachinehubException("%s signature failed for '%s' file."
